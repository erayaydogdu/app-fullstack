...
export function SignUpForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await fetch('/api/users/self-register', { 
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({ email, password, confirmPassword }) // Send necessary registration details
      });
      ...
      if (result.ok) {
        ...
        const errorData = await result.json().catch(() => ({ message: 'An unknown error occurred during sign up.' }));

...
function SignUpForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // For example, you can call your backend API to create a new user
      const backendToken = await getBackendToken(email, password);
      ...
      if (result.ok) {
        ...
        // You can redirect the user to the sign-in page or any other page
  ...
  return (
    <form onSubmit={handleSubmit} className="flex w-full max-w-md flex-col items-center justify-center space-y-6">
      ...
      <input
        ...
        className="w-full rounded-md border border-gray-200 px-4 py-2 text-black"
      />
      <input
        type="password"
